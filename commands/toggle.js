...
module.exports.make = async (bot, con) => {
        ...
        if (member.id == config.ownerID || roleList.indexOf(config.admin_role) != -1){
            ...
            if (args[0] === "all" || args[0] === "All") {
                ...
                for (let i = 0; i < commandRes.length; i++) {
                    ...
                    } catch (err) {contentString+= `Module ${commandRes[i].name.split('.')[0]} not enabled, ${err.code}: ${err.message}\n`}
                }
            ...
            for (let i = 0; i < args.length; i++) {
                if (isNaN(parseInt(args[i]))) { return} else {
                    ...
                    } catch (err) {contentString+= `Module ${commandRes[0].name.split('.')[0]} not enabled, ${err.code}: ${err.message}\n`}
                }
            }
            bot.createMessage(message.channel.id, {content: contentString})
            return
        } else { bot.createMessage(message.channel.id, {content:`You do not have the ${config.admin_role} role.`})}
    });
    bot.registerCommand("disable", async (message, args) => {
        ...
        if (member.id == config.ownerID || roleList.indexOf(config.admin_role) != -1){
            ...
            if (args[0] === "all" || args[0] === "All") {
                ...
                for (let i = 0; i < commandRes.length; i++) {
                    ...
                    } catch (err) {contentString+= `Module ${commandRes[i].name.split('.')[0]} not disabled, ${err.code}: ${err.message}\n`}
                }
                console.log(contentString)
                bot.createMessage(message.channel.id, {content: contentString})
                return
            ...
            for (let i = 0; i < args.length; i++) {
                if (isNaN(parseInt(args[i]))) { return} else {
                    ...
                    } catch (err) {contentString+= `Module ${commandRes[0].name.split('.')[0]} not disabled, ${err.code}: ${err.message}\n`}
                }
            }
            bot.createMessage(message.channel.id, {content: contentString})
            return
        ...
        let roleList = member.roles.map(r=>message.channel.guild.roles.get(r).name);
        if (member.id == config.ownerID || roleList.indexOf(config.admin_role) !== -1){
            con.query(`SELECT * FROM servers where code = ${message.channel.guild.id};`, (err, serverRes, fields) => {
                args.forEach(i => {
                    if (isNaN(parseInt(i))) {} else {
                        con.query(`SELECT * FROM commands where id = ${i};`, (err, commandRes, fields) => {
                            if (err) {
                                bot.createMessage(message.channel.id, {content: `An error occurred: ${err}`});
                                return
                            }
                            con.query(`DELETE FROM whiteListedCommands WHERE commandID=${commandRes[0].id} AND serverID=${serverRes[0].id};`, (err, whiteListRes, fields) => {
                                if (err) {
                                    bot.createMessage(message.channel.id, {
                                        content: `Module ${commandRes[0].name.split('.')[0]} not disabled, ${err}`
                                    });
                                    return
                                }
                                bot.createMessage(message.channel.id, {content: `Module ${commandRes[0].name.split('.')[0]} successfully disabled`})
                            })
                        })
                    }
                })
